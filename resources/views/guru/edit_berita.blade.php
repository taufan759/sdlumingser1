...
@section('content')
<p class="font-weight-bold">  <i class="fa-solid fa-newspaper"></i> Edit {{$berita->title}}</p>
<form method="POST" action="/guru/updated/berita/{{$berita->id}}" enctype="multipart/form-data">
    @csrf
    @if (session('success'))
    ...
    <div class="row mb-3">
        ...
        <div class="col-sm-10">
            <input id="title" type="text" class="form-control @error('title') is-invalid @enderror" name="title"
                value="{{ $berita->title}}" required placeholder="Judul Berita" autocomplete="title" autofocus>
            @error('title')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
    ...
    <div class="group relative z-0 mb-6 w-full">
        <textarea name="content" id="editor">{{$berita->content}}</textarea>
        @error('content')
